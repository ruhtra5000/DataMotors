...
router.get('/', (req, res) => {
	Orcamento.find({statusAberto: true}).lean().then((orcamentos) => {
...
router.get('/novo', (req, res) => {
	...
	//Realiza a busca de produtos
				Produto.find()
				.select('codigo descricao quantidade valorUnit').lean()
				.then((produtos) => {
					Produto.find()
					.select('codigo valorUnit')
					.then((produtos2) => {
	...
	Cliente.findOne({nome: req.body.cliente}).then((cliente) => {
		...
		if(req.body.arrayProdutos != ''){
			...
			for(produto of produtos){ 
				valorTotal += (Number(produto.valorUnit) * Number(produto.quantidade))
			}
		}
		...
		var newOrcamento = {
			...
			cliente: cliente._id,
			data: {
				dataEntrada: converterData(req.body.diaEntrada, req.body.mesEntrada, req.body.anoEntrada),
				dataSaida: converterData(req.body.diaSaida, req.body.mesSaida, req.body.anoSaida)
			},
			produtos: produtos,
			servicos: servicos,
		...
		new Orcamento(newOrcamento).save().then(() => {
			if(req.body.arrayProdutos != ''){
				for(produto of produtos){
					Produto.updateOne({codigo: produto.codigo}, {
						$inc: {
			...
			req.flash('err', 'Houve um erro interno ao salvar2. Tente novamente.' + err)
			res.redirect('/orcamento')
		})

	}).catch((err) => {
		req.flash('err', 'Houve um erro interno ao salvar3. Tente novamente.' + err)
		res.redirect('/orcamento')
	})
...
router.post('/editar', (req, res) => {
	...
	//Realiza a busca do cliente
						...
						.select('nome')
						.then((clientes) => {
							Cliente.findOne({_id: orcamento.cliente})
							.select('nome')
							.lean()
							.then((cliente) => {
	//Realiza a busca do responsável pelo orçamento
								Usuario.findOne({_id: orcamento.responsavel})
								.select('nome')
								.lean()
								.then((usuario) => {
									res.render('orcamento/editarOrcamento', {
										orcamento: orcamento,
										clientes: JSON.stringify(clientes),
										cliente: cliente,
										usuario: usuario,
...
router.post('/editarP', (req, res) => {
	Orcamento.findOne({_id: req.body.id}).lean().then((orcamento) => {
		Cliente.findOne({nome: req.body.cliente}).then((cliente) => {
			...
			Orcamento.updateOne({_id: req.body.id}, {
				$set: {
					descricao: req.body.descricao,
					cliente: cliente._id,
					data: {
				...
				if(req.body.arrayProdutos != ''){
					for(produto of produtos){
						Produto.updateOne({codigo: produto.codigo}, {
							...
							req.flash('suc', 'Orçamento editado com sucesso!')
							res.redirect('/orcamento')
						}).catch((err) => {
